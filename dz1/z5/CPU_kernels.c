...
void calculateLUT(float beta, float width, float **LUT, unsigned int *sizeLUT)
{
  ...
  if (width > 0)
  {
    ...
    unsigned int k;
    #pragma omp parallel for default(shared) private(k, v)
    for (k = 0; k < size; ++k)
    {
...
int gridding_Gold_parallel(unsigned int n, parameters params, ReconstructionSample *sample, float *LUT, unsigned int sizeLUT, cmplx* gridDataParallel, float *sampleDensity)
{
  ...
  #pragma omp parallel for private(NxL, NxH, NyL, NyH, NzL, NzH, dz2, nz, dx2, nx, dy2, ny, idxZ, idxY, dy2dz2, idx0, v, idx, w) 
                       //reduction(+: gridDataReal), reduction(+: gridDataImag), reduction(+: sampleDensity)
  for (i = 0; i < n; i++)
  {
    ReconstructionSample pt = sample[i];
            ...
            if (dy2dz2 < cutoff2)
            {
              for (dx2 = Dx2, nx = NxL; nx <= NxH; ++nx, ++dx2)
              {
                ...
                if (v < cutoff2)
                {
                  ...
                  #pragma omp atomic
                  gridDataParallel[idx].real += (w * pt.real);
                  #pragma omp atomic
                  gridDataParallel[idx].imag += (w * pt.imag);
